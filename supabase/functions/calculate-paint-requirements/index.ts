...
serve(async (req) => {
  ...
  try {
    ...
    if (typeof floor_area !== 'number' || typeof wall_area !== 'number' || 
      ...
      return new Response(JSON.stringify({ error: 'Area values must be numeric' }), {
        status: 400,
    ...
    if (floor_area < 0 || floor_area > 100000 || wall_area < 0 || wall_area > 100000 || 
      ...
      return new Response(JSON.stringify({ error: 'Area values must be positive and within reasonable ranges (0-100000 sq.ft)' }), {
        status: 400,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }
