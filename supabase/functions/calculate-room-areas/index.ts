...
serve(async (req) => {
  ...
  try {
    ...
    if (length < 0.1 || length > 10000 || width < 0.1 || width > 10000 || height < 0 || height > 100) {
      return new Response(JSON.stringify({ error: 'Dimensions must be positive and within reasonable ranges (length/width: 0.1-10000, height: 0-100)' }), {
        status: 400,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }
    ...
    if (opening_areas.length > 50 || extra_surfaces.length > 50 || door_window_grills.length > 50) {
      return new Response(JSON.stringify({ error: 'Too many area entries specified (max 50 per type)' }), {
        status: 400,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }
    ...
    const validateAreaArray = (arr: Array<{ area: number }>, name: string) => {
      for (const item of arr) {
        if (typeof item.area !== 'number' || item.area < 0 || item.area > 10000) {
          throw new Error(`${name} contains invalid area values`);
        }
      }
    };

    validateAreaArray(opening_areas, 'opening_areas');
    validateAreaArray(extra_surfaces, 'extra_surfaces');
    validateAreaArray(door_window_grills, 'door_window_grills');
