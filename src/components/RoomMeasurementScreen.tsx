...
export default function RoomMeasurementScreen() {
  ...
  const [newExtraSurface, setNewExtraSurface] = useState({
    height: "",
    width: "",
    quantity: ""
  });
  ...
  const [roomExtraSurfaceInputs, setRoomExtraSurfaceInputs] = useState<Record<string, {
    height: string;
    width: string;
    quantity: string;
  }>>({});
    ...
    const loadData = async () => {
      try {
        ...
        const {
          data: roomsData,
          error: roomsError
        } = await supabase.from('rooms').select('*').eq('project_id', projectId).order('created_at', {
          ascending: true
        });
        if (roomsError) {
        ...
        if (roomsData && roomsData.length > 0) {
          const formattedRooms: Room[] = roomsData.map(room => ({
            id: room.room_id,
            name: room.name,
            sectionName: (room as any).section_name || undefined,
            ...
            selectedAreas: typeof room.selected_areas === 'object' && room.selected_areas !== null ? room.selected_areas as {
              floor: boolean;
              wall: boolean;
              ceiling: boolean;
            } : {
  ...
  const calculateAreasInBackground = useCallback(async (length: number, width: number, height: number, openingAreas: OpeningArea[], extraSurfaces: ExtraSurface[], doorWindowGrills: DoorWindowGrill[]) => {
    try {
      const {
        data,
        error
      } = await supabase.functions.invoke('calculate-room-areas', {
        body: {
          length,
          width,
          height,
  ...
  const calculateAreas = useCallback((length: number, width: number, height: number, openingAreas: OpeningArea[], extraSurfaces: ExtraSurface[], doorWindowGrills: DoorWindowGrill[]) => {
  ...
  const addRoom = async () => {
    ...
    if (newRoom.name && newRoom.length && newRoom.width && newRoom.pictures.length >= 2) {
      ...
      const roomData = {
        ...
        // Custom sections created via (+) icon
  ...
  const removeRoom = useCallback(async (roomId: string) => {
    try {
      const {
        error
      } = await supabase.from('rooms').delete().eq('room_id', roomId).eq('project_id', projectId!);
      if (error) {
  ...
  const addOpeningAreaToRoom = useCallback(async (roomId: string) => {
    const roomInput = roomOpeningInputs[roomId] || {
      height: "",
      width: "",
      quantity: ""
    };
    ...
    const areas = calculateAreas(targetRoom.length, targetRoom.width, targetRoom.height, updatedOpeningAreas, targetRoom.extraSurfaces, targetRoom.doorWindowGrills);
  ...
  const addExtraSurfaceToRoom = useCallback(async (roomId: string) => {
    const roomInput = roomExtraSurfaceInputs[roomId] || {
      height: "",
      width: "",
      quantity: ""
    };
    ...
    const areas = calculateAreas(targetRoom.length, targetRoom.width, targetRoom.height, targetRoom.openingAreas, updatedExtraSurfaces, targetRoom.doorWindowGrills);
  ...
  const handleSaveSubArea = useCallback(async (subArea: SubArea) => {
    ...
    if (editingSubArea) {
      ...
      updatedSubAreas = targetRoom.subAreas.map(sa => sa.id === subArea.id ? subArea : sa);
    } else {
      // Add new sub-area
    ...
    setRooms(prev => prev.map(room => room.id === subAreaRoomId ? {
      ...room,
    ...
    try {
      const {
        ...
        sub_areas: updatedSubAreas as any
      }).eq('room_id', subAreaRoomId).eq('project_id', projectId!);
      if (error) {
  ...
  const handleSaveCustomSection = useCallback(() => {
    ...
    if (editingCustomSectionId) {
      // Update existing
      setTempCustomSections(prev => prev.map(s => s.id === editingCustomSectionId ? {
        ...s,
  ...
  const handleAddSection = useCallback(async () => {
    ...
    setRooms(prev => prev.map(r => r.id === addSectionRoomId ? {
      ...r,
    ...
    try {
      ...
      }).eq('room_id', addSectionRoomId).eq('project_id', projectId!);
      if (error) {
  ...
  const handleAddSeparateSection = useCallback(async () => {
    ...
    // Update local state immediately
    setRooms(prev => prev.map(room => room.id === separateSectionRoomId ? {
      ...room,
      subAreas: updatedSubAreas
    } : room));
    ...
    try {
      const {
        ...
        sub_areas: updatedSubAreas as any
      }).eq('room_id', separateSectionRoomId).eq('project_id', projectId!);
      if (error) {
    ...
    if (targetRoom) {
      ...
      try {
        const {
          ...
          adjusted_wall_area: areas.adjustedWallArea
        }).eq('room_id', targetRoom.id).eq('project_id', projectId!);
        if (error) {
          console.error('Error updating room:', error);
          toast.error('Failed to add door/window');
          return;
        }
        setRooms(prev => prev.map(room => room.id === targetRoom!.id ? {
          ...room,
          doorWindowGrills: updatedDoorWindowGrills,
          ...areas
        } : room));
      ...
      try {
        const {
          ...
          room_id: roomId,
          name: newRoom.name,
          section_name: sectionNameTrimmed || null,
          // Section header for Paint Estimation
  ...
  const updateDoorWindowGrillName = useCallback((roomId: string, dwgId: string, newName: string) => {
    const targetRoom = rooms.find(r => r.id === roomId);
    if (!targetRoom) return;
    const updatedDoorWindowGrills = targetRoom.doorWindowGrills.map(dwg => dwg.id === dwgId ? {
      ...dwg,
      name: newName
    } : dwg);
  ...
  const completeRoom = useCallback(async (roomId: string) => {
    ...
    try {
      ...
      const {
        data,
        error
      } = await supabase.functions.invoke('calculate-paint-requirements', {
        body: {
          room_id: roomId,
          floor_area: room.floorArea,
          wall_area: room.wallArea,
      ...
      toast.success(`Room completed! (${elapsedTime}ms)`, {
        duration: 2000
      });
    } catch (error) {
  ...
  const handleUpdateRoom = async () => {
    ...
    const areas = calculateAreas(length, width, height, editingRoom.openingAreas, editingRoom.extraSurfaces, editingRoom.doorWindowGrills);
  ...
  return <div className="min-h-screen bg-background">
      ...
      <div className="eca-gradient text-white p-4">
        <div className="flex items-center space-x-3">
          <Button variant="ghost" size="icon" className="text-white hover:bg-white/20" onClick={() => navigate("/add-project")}>
      ...
      <div className="p-4 space-y-6">
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          ...
          <TabsContent value="sqft" className="space-y-6">
            ...
            {projectData && <Card className="eca-shadow">
                <CardHeader>
                ...
                <CardContent>
                  <div className="flex flex-wrap gap-2">
                    {projectData.projectTypes.map(type => <Button key={type} variant={activeProjectType === type ? "default" : "outline"} onClick={() => {
                  setActiveProjectType(type);
                  try {
                    localStorage.setItem(`selected_paint_type_${projectId}`, type);
                  } catch {}
                }} className="text-sm">
                        {type}
                      </Button>)}
            ...
            {/* Add New Room */}
            {activeProjectType && <Card className="eca-shadow">
                <CardHeader>
                ...
                <CardContent className="space-y-4">
                  <div className="space-y-2">
                    ...
                    <Input placeholder="Name of the Area" value={newRoom.name} onChange={e => setNewRoom(prev => ({
                  ...prev,
                  name: e.target.value
                }))} className="h-12" />
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      <Button type="button" variant="outline" onClick={() => fileInputRef.current?.click()} className="h-12" disabled={newRoom.pictures.length >= 5}>
                        <Image className="mr-2 h-4 w-4" />
                        Choose File
                      </Button>
                      <Button type="button" variant="outline" onClick={() => cameraInputRef.current?.click()} className="h-12" disabled={newRoom.pictures.length >= 5}>
                        <Camera className="mr-2 h-4 w-4" />
                        Take Photo
                      </Button>
                    </div>
                    <input ref={fileInputRef} type="file" accept="image/*" multiple className="hidden" onChange={e => handlePictureUpload(e.target.files, false)} />
                    <input ref={cameraInputRef} type="file" accept="image/*" capture="environment" className="hidden" onChange={e => handlePictureUpload(e.target.files, true)} />
                    
                    {newRoom.pictures.length > 0 && <div className="grid grid-cols-5 gap-2 mt-2">
                        {newRoom.pictures.map((pic, index) => <div key={index} className="relative aspect-square rounded-lg overflow-hidden border border-border">
                            <img src={pic} alt={`Room ${index + 1}`} className="w-full h-full object-cover" />
                            <Button variant="destructive" size="icon" className="absolute top-1 right-1 h-6 w-6" onClick={() => removePicture(index)}>
                              <X className="h-3 w-3" />
                            </Button>
                          </div>)}
                      </div>}
                  ...
                  {!showOpenAreaSection ? <Button onClick={handleAddRoomClick} disabled={!newRoom.name || !newRoom.length || !newRoom.width || newRoom.pictures.length < 2} className="w-full h-12">
                      ...
                      <div className="space-y-3 p-4 bg-red-50 dark:bg-red-900/20 rounded-lg border border-red-200 dark:border-red-800">
                        ...
                        {tempOpeningAreas.map(opening => <div key={opening.id} className="flex items-center justify-between p-2 bg-background rounded">
                            <span className="text-sm">{opening.height} × {opening.width} × {opening.quantity} = {opening.area.toFixed(1)} sq.ft</span>
                            <Button variant="ghost" size="icon" className="h-6 w-6" onClick={() => removeTempOpeningArea(opening.id)}>
                              <X className="h-3 w-3" />
                            </Button>
                          </div>)}
                      ...
                      <div className="space-y-3 p-4 bg-green-50 dark:bg-green-900/20 rounded-lg border border-green-200 dark:border-green-800">
                        ...
                        {tempExtraSurfaces.map(extra => <div key={extra.id} className="flex items-center justify-between p-2 bg-background rounded">
                            <span className="text-sm">{extra.height} × {extra.width} × {extra.quantity} = {extra.area.toFixed(1)} sq.ft</span>
                            <Button variant="ghost" size="icon" className="h-6 w-6" onClick={() => removeTempExtraSurface(extra.id)}>
                              <X className="h-3 w-3" />
                            </Button>
                          </div>)}
                      ...
                      {tempCustomSections.length > 0 && <div className="space-y-3 p-4 bg-primary/5 rounded-lg border border-primary/20">
                          <h4 className="font-semibold text-sm text-primary">Separate Paint Sections (will appear with Floor/Wall/Ceiling)</h4>
                          <div className="grid grid-cols-3 gap-3">
                            {tempCustomSections.map(section => <div key={section.id} className="p-3 rounded-lg border-2 border-primary bg-primary/10 relative group">
                                <div className="absolute top-1 right-1 flex gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity">
                                  <Button variant="ghost" size="icon" className="h-5 w-5" onClick={() => handleOpenCustomSectionDialog(section.id)}>
                                    <Edit3 className="h-3 w-3" />
                                  </Button>
                                  <Button variant="ghost" size="icon" className="h-5 w-5 text-destructive hover:text-destructive" onClick={() => handleRemoveCustomSection(section.id)}>
                                    <X className="h-3 w-3" />
                                  </Button>
                                </div>
                                <div className="text-center">
                      ...
                      <Button onClick={addRoom} className="w-full h-12">
                        Save Room
                      </Button>
                    </>}
                </CardContent>
                        ...
                        <div className="space-y-4">
                          ...
                          <div className="flex items-center justify-between">
                            ...
                            <div className="flex items-center space-x-1">
                              <Button variant="default" size="sm" className="h-9 hidden" onClick={() => completeRoom(room.id)}>
                                <Calculator className="h-4 w-4 mr-1" />
                                Complete
                              </Button>
                              <Button variant="ghost" size="icon" className="h-9 w-9 text-primary hover:text-primary hover:bg-primary/10" onClick={() => {
                            ...
                            setAddSectionDialogOpen(true);
                              ...
                              <Button variant="ghost" size="icon" className="h-9 w-9" onClick={() => handleEditRoom(room)}>
                                <Edit3 className="h-4 w-4" />
                              </Button>
                              <Button variant="ghost" size="icon" className="h-9 w-9 text-destructive" onClick={() => removeRoom(room.id)}>
                          ...
                          {/* Room Pictures */}
                          {room.pictures && room.pictures.length > 0 && <div className="grid grid-cols-5 gap-2">
                              {room.pictures.map((pic, index) => <div key={index} className="aspect-square rounded-lg overflow-hidden border border-border">
                          ...
                          <div className="space-y-2 p-3 bg-red-50 dark:bg-red-900/20 rounded-lg border border-red-200 dark:border-red-800">
                            ...
                            {room.openingAreas.map(opening => <div key={opening.id} className="flex items-center justify-between text-sm">
                                <span className="text-foreground">
                                  {opening.height} × {opening.width} × {opening.quantity} = {opening.area.toFixed(1)} sq.ft
                                </span>
                                <Button variant="ghost" size="icon" className="h-6 w-6 text-red-700 dark:text-red-400" onClick={() => removeOpeningArea(room.id, opening.id)}>
                                  <X className="h-3 w-3" />
                                </Button>
                              </div>)}
                          ...
                          }))} className="h-9" step="1" />
                            </div>
                            <Button variant="outline" size="sm" onClick={() => addOpeningAreaToRoom(room.id)} disabled={!roomOpeningInputs[room.id]?.height || !roomOpeningInputs[room.id]?.width} className="w-full">
                              <Plus className="mr-1 h-3 w-3" />
                              Add Opening
                            </Button>
                          </div>
                          ...
                          <div className="space-y-2 p-3 bg-green-50 dark:bg-green-900/20 rounded-lg border border-green-200 dark:border-green-800">
                            ...
                            {room.extraSurfaces.map(extra => <div key={extra.id} className="flex items-center justify-between text-sm">
                                <span className="text-foreground">
                                  {extra.height} × {extra.width} × {extra.quantity} = {extra.area.toFixed(1)} sq.ft
                                </span>
                                <Button variant="ghost" size="icon" className="h-6 w-6 text-green-700 dark:text-green-400" onClick={() => removeExtraSurface(room.id, extra.id)}>
                                  <X className="h-3 w-3" />
                                </Button>
                              </div>)}
                            ...
                            <Button variant="outline" size="sm" onClick={() => addExtraSurfaceToRoom(room.id)} disabled={!roomExtraSurfaceInputs[room.id]?.height || !roomExtraSurfaceInputs[room.id]?.width} className="w-full">
                              <Plus className="mr-1 h-3 w-3" />
                              Add Extra Surface
                            </Button>
                          </div>
                          ...
                          <div className="grid grid-cols-3 gap-3">
                            {/* Floor Area */}
                            <div className={`p-3 rounded-lg border-2 cursor-pointer transition-all ${room.selectedAreas.floor ? 'border-blue-500 bg-blue-50 dark:bg-blue-900/20' : 'border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/20'}`} onClick={() => toggleAreaSelection(room.id, 'floor')}>
                              <div className="text-center">
                            ...
                            {/* Wall Area */}
                            <div className={`p-3 rounded-lg border-2 cursor-pointer transition-all ${room.selectedAreas.wall ? 'border-green-500 bg-green-50 dark:bg-green-900/20' : 'border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/20'}`} onClick={() => toggleAreaSelection(room.id, 'wall')}>
                              <div className="text-center">
                            ...
                            {/* Ceiling Area */}
                            <div className={`p-3 rounded-lg border-2 cursor-pointer transition-all ${room.selectedAreas.ceiling ? 'border-purple-500 bg-purple-50 dark:bg-purple-900/20' : 'border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/20'}`} onClick={() => toggleAreaSelection(room.id, 'ceiling')}>
                              <div className="text-center">
                          ...
                          {room.subAreas && room.subAreas.length > 0 && <div className="space-y-2 p-3 bg-primary/5 rounded-lg border border-primary/20">
                              <div className="flex items-center justify-between">
                                <h4 className="text-sm font-semibold text-primary">Sub-Areas</h4>
                              </div>
                              <div className="grid grid-cols-3 gap-3">
                                {room.subAreas.map(subArea => <div key={subArea.id} className="p-3 rounded-lg border-2 border-primary bg-primary/10 relative group cursor-pointer transition-all">
                                    <div className="absolute top-1 right-1 flex gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity">
                                      <Button variant="ghost" size="icon" className="h-5 w-5" onClick={e => {
                                e.stopPropagation();
                                handleEditSubArea(room.id, subArea);
                              }}>
                                        <Edit3 className="h-3 w-3" />
                                      </Button>
                                      <Button variant="ghost" size="icon" className="h-5 w-5 text-destructive hover:text-destructive" onClick={e => {
                                e.stopPropagation();
                                handleRemoveSubArea(room.id, subArea.id);
                              }}>
                                        <X className="h-3 w-3" />
                                      </Button>
                                    </div>
          ...
          <TabsContent value="doorwindow" className="space-y-6">
            ...
            {projectData && projectData.projectTypes.length > 0 && <Card className="eca-shadow">
                <CardHeader>
                  <CardTitle className="flex items-center text-lg">
                    <Home className="mr-2 h-5 w-5 text-primary" />
                    Select Project Type
                ...
                <CardContent>
                  <div className="flex flex-wrap gap-3">
                    {projectData.projectTypes.map(type => <Button key={type} variant={activeProjectType === type ? "default" : "outline"} onClick={() => setActiveProjectType(type)} className="h-12 px-6">
                        {type}
                      </Button>)}
                  </div>
                </CardContent>
                    ...
                    {getRoomsByProjectType(activeProjectType).map(room => <div key={`dwg-${room.id}`} className="space-y-0">
                        ...
                        <CardContent className="p-4">
                              ...
                              <div className="flex items-center justify-between mb-2">
                                <div className="flex items-center space-x-2 flex-1">
                                  <Input 
                                    ...
                                    className="font-medium bg-transparent border-none h-8 p-0 focus:ring-1 focus:ring-primary" 
                                  />
                                  <button 
                                    type="button"
                                    onClick={() => {
                                      const input = document.getElementById(`dwg-input-${dwg.id}`) as HTMLInputElement;
                                      if (input) {
                                        input.focus();
                                        input.select();
                                      }
                          ...
                          <div className="grid grid-cols-2 gap-3 mb-3">
                            <Input placeholder="e.g., Main Door, Window 1" value={newDoorWindowGrill[room.id]?.name || ""} onChange={e => setNewDoorWindowGrill(prev => ({
                          ...prev,
                          [room.id]: {
                            ...(prev[room.id] || {
                              name: "",
                            ...
                            <Input type="number" placeholder="Sides" value={newDoorWindowGrill[room.id]?.sides || ""} onChange={e => setNewDoorWindowGrill(prev => ({
                          ...prev,
                          [room.id]: {
                            ...(prev[room.id] || {
                              name: "",
                          ...
                          <Button variant="outline" size="sm" className="w-full" onClick={() => addDoorWindowGrillToRoom(room.id)} disabled={!newDoorWindowGrill[room.id]?.height || !newDoorWindowGrill[room.id]?.width || !newDoorWindowGrill[room.id]?.sides}>
                            <Plus className="mr-1 h-3 w-3" />
                            Add to {room.name}
                          </Button>
                        </CardContent>
        ...
        <DialogContent className="sm:max-w-md">
          ...
          <div className="space-y-4">
            <div className="space-y-2">
              ...
              <Input id="edit-name" placeholder="e.g., Living Room, Bedroom" value={editFormData.name} onChange={e => setEditFormData(prev => ({
              ...prev,
              name: e.target.value
            }))} className="h-12" />
            </div>
            ...
            <div className="flex gap-3">
              ...
              <Button className="flex-1" onClick={handleUpdateRoom} disabled={!editFormData.name || !editFormData.length || !editFormData.width}>
                Save Changes
              </Button>
            </div>
          </div>
      ...
      <Dialog open={doorWindowDialogOpen} onOpenChange={setDoorWindowDialogOpen}>
        <DialogContent className="sm:max-w-md">
          ...
          <div className="space-y-4">
            {/* Project Type Selection */}
            {projectData && projectData.projectTypes.length > 1 && <div className="space-y-2">
                <Label>Project Type</Label>
                <div className="flex flex-wrap gap-2">
                  {projectData.projectTypes.map(type => <Button key={type} variant={doorWindowProjectType === type ? "default" : "outline"} size="sm" onClick={() => setDoorWindowProjectType(type)} className="h-10">
                      {type}
                    </Button>)}
                </div>
              </div>}
      ...
      {/* Sub-Area Dialog */}
      <SubAreaDialog open={subAreaDialogOpen} onClose={() => {
      setSubAreaDialogOpen(false);
      setSubAreaRoomId(null);
      setEditingSubArea(null);
    }} onSave={handleSaveSubArea} editingSubArea={editingSubArea} />
        ...
        <DialogContent className="sm:max-w-[350px]">
          ...
          <div className="space-y-4 py-4">
            ...
            <div className="space-y-2">
              ...
              <Input id="custom-section-name" placeholder="e.g., Damp Wall, Texture Wall, Accent Wall" value={customSectionName} onChange={e => setCustomSectionName(e.target.value)} className="h-12" />
            </div>
          </div>
        ...
        <DialogContent className="sm:max-w-[350px]">
          ...
          <div className="space-y-4 py-4">
            ...
            <div className="space-y-2">
              ...
              <Input id="separate-section-name" placeholder="e.g., Damp Wall, Texture Section, First Floor Wall" value={separateSectionName} onChange={e => setSeparateSectionName(e.target.value)} className="h-12" autoFocus />
            </div>
          </div>
          <div className="flex justify-end gap-2">
            <Button variant="outline" onClick={() => {
