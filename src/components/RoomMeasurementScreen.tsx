...
export default function RoomMeasurementScreen() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        const { data: projectDataFromDb, error: projectError } = await supabase
          .from('projects')
        ...
        if (projectDataFromDb) {
          // Parse project_type string (comma-separated) into array
          const projectTypes = projectDataFromDb.project_type
            .split(',')
            .map((type: string) => type.trim())
            .filter(Boolean);
          
          const data = {
            customerName: projectDataFromDb.customer_name || '',
            mobile: projectDataFromDb.phone || '',
            address: projectDataFromDb.location || '',
            projectTypes
          };
  ...
  const addRoom = async () => {
    ...
    if (newRoom.name && newRoom.length && newRoom.width && newRoom.pictures.length >= 2) {
      ...
      // Calculate areas in background using edge function
      (async () => {
        ...
        try {
          const { error } = await supabase
            ...
            .update({
              ...
              width,
              height,
              floor_area: calculatedAreas.floor_area,
              wall_area: calculatedAreas.wall_area,
              ceiling_area: calculatedAreas.ceiling_area,
              adjusted_wall_area: calculatedAreas.adjusted_wall_area,
              total_opening_area: calculatedAreas.total_opening_area,
              total_extra_surface: calculatedAreas.total_extra_surface,
              total_door_window_grill_area: calculatedAreas.total_door_window_grill_area
            })
